...
const ADMIN_PASS    = 'edna2026';

let sb = null; // Supabase client – initialized only after login
let allBookings = [];

/* ── INIT Supabase (called after login) ── */
function initSupabase() {
  ...
  try {
    ...
    alert('שגיאה בטעינת Supabase. נסי לרענן את הדף.');
    return false;
  }
}
...
function checkPassword() {
  ...
  if (!input || input.value.trim() !== ADMIN_PASS) {
...
async function loadBookings() {
  const { data } = await sb.from('bookings').select('*').order('date').order('time');
  allBookings = data || [];
  updateTodayCount();
...
function updateTodayCount() {
  ...
  if (el) el.textContent = `היום: ${n} תורים`;
}
...
async function uploadImage(settingKey, fileInputId, previewId, emptyId, statusId) {
  ...
  if (!allowed.includes(file.type)) {
    status.textContent = 'סוג קובץ לא נתמך. השתמשי ב-JPG/PNG/WebP';
  ...
  const path = `${settingKey}-${Date.now()}.${file.name.split('.').pop()}`;
  ...
  const { error: dbErr } = await sb.from('settings').upsert({ key: settingKey, value: publicUrl }, { onConflict: 'key' });
